...
async def kubectl_delete(ops_test, unit, wait: bool = True) -> None:
...
def scale_application(
    juju: jubilant.Juju, application_name: str, desired_count: int, wait: bool = True
) -> None:
    ...
    Args:
        ...
        wait: Boolean indicating whether to wait until units
            reach desired count.
    """
    if len(juju.status().apps[application_name].units) == desired_count:
        return

    juju.cli("scale-application", application_name, f"{desired_count}")

    if desired_count > 0 and wait:
        juju.wait(
            lambda status: jubilant_all_units_idle(status, apps=[application_name])
            and jubilant.all_active(status, apps=[application_name]),
            timeout=15 * 60,
            delay=10,
        )

    assert len(juju.status().apps[application_name].units) == desired_count


def get_address(juju: jubilant.Juju, app_name, unit_num=0) -> str:
